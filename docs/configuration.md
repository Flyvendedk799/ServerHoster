...
| `SURVHUB_AUTH_TOKEN`              | string     | _(empty)_                     | Optional static bearer token. When set, `Authorization: Bearer <token>` is always accepted regardless of session state. Useful for CI.                                                                                                                                                                                                                                                                       |
| `SURVHUB_SECRET_KEY`              | string     | _(empty)_                     | **Required in production.** Used as the key for AES-256-GCM encryption of every secret stored at rest (service env vars, GitHub PAT, Cloudflare tokens). Losing this key = losing the data.                                                                                                                                                                                                                  |
...
| `SURVHUB_TRUST_PROXY`             | `1` / list | `0`                           | Believe `X-Forwarded-For`. Set to `1` behind cloudflared/nginx/Caddy, or to a comma-separated list of proxy addresses or CIDRs. Without it, every request through a proxy shares one `req.ip`: per-IP rate limits become one global bucket and a paired phone's last-seen IP is the tunnel's. Leave off when exposed directly — trusting the header with no proxy in front lets callers forge their address. |
...
├── resources/          # Managed resource state, one folder per resource id
│   └── <resourceId>/
│       └── supabase/
│           └── .env    # Edge Function env file (mode 0600, regenerated from resource_secrets)
...
| `managed_resources`      | `id`, `project_id`, `name`, `profile` (`supabase`/`postgres`/`redis`/`manual`), `status`, `config_json`, `ports_json`, `containers_json`                                                                                                                                                                                                                             |
| `resource_secrets`       | `id`, `resource_id`, `key`, `value` (**always** encrypted), `is_generated`, unique `(resource_id, key)`                                                                                                                                                                                                                                                              |
| `service_resource_links` | `id`, `service_id`, `resource_id`, `active`, `env_map_json`, unique `(service_id, resource_id)`                                                                                                                                                                                                                                                                      |
| `dependency_scans`       | `id`, `service_id`, `profile` (recommended), `confidence`, `signals_json`, `env_requirements_json`, `created_at` (insert-only; latest row per service wins)                                                                                                                                                                                                          |
...
- `cloudflare_account_id` _(sensitive; encrypted)_

All other settings keys are stored as plain text.

**Managed resource layer** (dependency-aware provisioning): the four `managed_resources` / `resource_secrets` / `service_resource_links` / `dependency_scans` tables sit alongside the legacy `databases` table — `databases` and `services.linked_database_id` are untouched for backward compatibility, and a `postgres` resource also writes a legacy `databases` row. `resource_secrets.value` is **always** encrypted with the same AES-256-GCM path keyed by `SURVHUB_SECRET_KEY`; API responses only ever expose masked previews, and the Supabase service-role key, JWT secret, and internal DB URL are never returned at all.

Env injection precedence for a running service:

```
project env → active resource links → legacy linked DATABASE_URL → DATA_DIR → service env
```

Service-level env always wins. The same merged env feeds both build and runtime, so a static/Vite service is rebuilt (not just restarted) when a resource link changes its `VITE_*` values.
