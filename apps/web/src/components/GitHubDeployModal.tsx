    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" style={{ maxWidth: "640px" }} onClick={(e) => e.stopPropagation()}>
           ...
           {step === 1 ? (
             <div className="form-group">
                <div className="row">
                   <input 
                    ...
                    style={{ flex: 1 }}
                   />
                   <button className="ghost" onClick={loadRepos} disabled={loading}>Refresh List</button>
                </div>
                
                <div className="repo-list" style={{ marginTop: "1rem", maxHeight: "300px", overflowY: "auto", border: "1px solid var(--border-default)", borderRadius: "var(--radius-md)" }}>
                   {repos.length === 0 && !loading && (
                     <div className="muted small text-center" style={{ padding: "2rem" }}>
                        Click Refresh to load your repositories.
                     </div>
                   )}
                   {loading && <div className="muted small text-center" style={{ padding: "2rem" }}>Scanning universe...</div>}
                   {filtered.map(repo => (
                     <div 
                      key={repo.full_name} 
                      className="repo-item" 
                      onClick={() => selectRepo(repo)}
                      style={{ padding: "1rem", borderBottom: "1px solid var(--border-subtle)", cursor: "pointer", transition: "var(--transition)" }}
                     >
             ...
             <div className="form-column" style={{ display: "grid", gap: "1.5rem" }}>
                ...
                <label className="toggle-group">
                   <input type="checkbox" checked={form.autoPull} onChange={e => setForm({...form, autoPull: e.target.checked})} />
        ...
        <footer className="modal-footer">
           ...
           {step === 2 && <button className="ghost" onClick={() => setStep(1)}>Back</button>}
           {step === 1 && <button className="primary" onClick={() => setStep(2)} disabled={!form.repoUrl}>Next Step</button>}
           {step === 2 && <button className="primary" onClick={handleDeploy} disabled={deploying}>
