...
export function SettingsPage() {
  const [activeTab, setActiveTab] = useState<"connectivity" | "integrations" | "data" | "system">("connectivity");
  ...
  async function loadAll() {
    try {
      ...
      setSshInfo(ssh);
      setTunnel(t);
      if (t) setCfConfig({ accountId: t.accountId ?? "", tunnelId: t.tunnelId ?? "", zoneId: t.zoneId ?? "" });
    } catch { /* silent */ }
  }
  ...
  async function saveGithubPat() {
    try {
      await api("/settings/github/pat", { method: "POST", body: JSON.stringify({ token: githubTokenInput }) });
  ...
  return (
    <div className="settings-page">
      <header className="page-header">
        ...
        <button className="ghost logout" onClick={() => { clearAuthToken(); window.location.href = "/login"; }}>
        ...
        <aside className="settings-nav">
          {tabs.map(tab => (
            ...
            >
              ...
              <ChevronRight size={14} className="muted ml-auto" style={{ opacity: activeTab === tab.id ? 1 : 0 }} />
            </button>
          ))}
        </aside>
              ...
              {activeTab === "connectivity" && (
                <div className="form-stack">
                  <div className="card glass-card">
                    ...
                    <p className="muted small" style={{ marginBottom: "2rem" }}>Connect your local node to the Cloudflare edge without opening ingress ports.</p>
                    
                    <div className="form-row">
                       <div className="form-group">
                         <label className="tiny uppercase font-bold muted">Account Pointer (ID)</label>
                         <input value={cfConfig.accountId} onChange={e => setCfConfig({...cfConfig, accountId: e.target.value})} placeholder="32-character ID" />
                       </div>
                       <div className="form-group">
                         <label className="tiny uppercase font-bold muted">Tunnel Identifier</label>
                         <input value={cfConfig.tunnelId} onChange={e => setCfConfig({...cfConfig, tunnelId: e.target.value})} placeholder="UUID" />
                       </div>
                    </div>
                    ...
                    <footer className="footer-actions">
                      ...
                      <button className="ghost text-danger" onClick={() => api("/cloudflare/stop", { method: "POST" }).then(loadAll)}>
                  ...
                  <div className="card glass-card">
                    ...
                    <p className="muted small" style={{ margin: "1rem 0" }}>Configure how SURVHub issues SSL/TLS certificates via Let's Encrypt.</p>
                    <div className="form-group">
                    ...
                    <button className="button small" style={{ marginTop: "1.5rem" }} onClick={() => api("/settings", { method: "PUT", body: JSON.stringify({ key: "ssl_mode", value: sslMode }) })}>
                       Save Strategy
                    </button>
                  </div>
                </div>
              ...
              {activeTab === "integrations" && (
                <div className="form-stack">
                      ...
                      <div className="row small" style={{ marginTop: "1rem" }}>
                        {githubStatus?.configured ? (
                           <>
                             ...
                             <span className="muted">Authenticated as <code className="text-accent">{githubStatus.tokenPrefix}***</code></span>
                           </>
                        ) : (
                           <>
                             <AlertCircle size={14} className="text-warning" />
                             <span className="muted">Connect to enable private repository synchronization.</span>
                           </>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="card glass-card">
                    ...
                    <p className="muted small" style={{ margin: "1rem 0" }}>This public key is used for Git clones and secure cluster communication.</p>
                    <div className="ssh-box">
                      <code>{sshInfo?.publicKey || "Generating keys..."}</code>
                    </div>
                    <button className="ghost small font-bold" onClick={() => sshInfo?.publicKey && navigator.clipboard.writeText(sshInfo.publicKey).then(() => toast.success("Key copied to buffer"))}>
              ...
              {activeTab === "data" && (
                <div className="form-stack">
                   <div className="card glass-card">
                      ...
                      <p className="muted small" style={{ margin: "1rem 0" }}>Export all configuration, routing rules, and service metadata as a portable JSON file.</p>
                      <button className="primary" onClick={() => api("/backup/export").then(d => {
                        const blob = new Blob([JSON.stringify(d, null, 2)], { type: "application/json" });
                        const url = URL.createObjectURL(blob);
                        const a = document.createElement("a");
                        a.href = url; a.download = `survhub-export-${new Date().toISOString().slice(0, 19)}.json`;
                        a.click();
              ...
              {activeTab === "system" && (
                <div className="form-stack">
                   <div className="card glass-card">
                      ...
                      <p className="muted small" style={{ marginTop: "1rem" }}>Execute maintenance tasks directly on the node control plane.</p>
                      <div className="row wrap" style={{ gap: "1rem", marginTop: "2rem" }}>
                         <button className="button" onClick={() => api("/ops/audit-logs").then(() => toast.success("Audit pushed to syslogs"))}>
                            <Terminal size={16} /> Audit Log Stream
                         </button>
                         <button className="button" onClick={() => api("/ops/install-scripts").then(() => toast.success("Setup wizard cached"))}>
