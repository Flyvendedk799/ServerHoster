...
export function DeploymentsPage() {
    ...
    const ws = connectLogs((payload) => {
      ...
      if (typed.type === "build_log" && typed.deploymentId) {
        setLiveLogs((prev) => {
          ...
          return { ...prev, [typed.deploymentId!]: [...existing, { line: typed.line!, stream: typed.stream ?? "stdout" }].slice(-2000) };
        });
  ...
  return (
    <div className="deployments-page">
      ...
      <section className="card featured-form" style={{ marginBottom: "4rem", border: "1px solid var(--border-glow)" }}>
      ...
      {running.length > 0 && (
        <section className="card active-pipeline" style={{ marginBottom: "4rem", border: "1px solid var(--accent)", background: "rgba(59,130,246,0.05)" }}>
          <div className="section-title">
            <div className="row">
              ...
              >
                <div className="terminal-header row between">
                   <div className="row small">
                      ...
                      <span className="text-accent uppercase tiny font-bold">{phases[d.id] ?? "Initializing"}</span>
                   </div>
                   <div className="row">
                      <button className="ghost xsmall" onClick={() => toast.info("Full-screen logs coming soon")}><Maximize2 size={12} /></button>
                   </div>
                </div>
                <div className="logs-viewer terminal" ref={terminalRef}>
                  ...
                  {(!liveLogs[d.id] || liveLogs[d.id].length === 0) && (
                     <div className="muted small" style={{ padding: "1rem" }}>Awaiting container orchestrator...</div>
                  )}
                </div>
              </motion.div>
            ))}
              ...
              >
                <div className="service-header">
                  <div className="service-title-group">
                    <h3 className="small">{services.find(s => s.id === d.service_id)?.name ?? "Legacy Resource"}</h3>
                  ...
                  <div className="row">
                     {d.status === "success" ? <CheckCircle2 size={18} className="text-success" /> : d.status === "failed" ? <XCircle size={18} className="text-danger" /> : <Loader2 size={18} className="animate-spin text-accent" />}
                     <StatusBadge status={d.status} />
                  </div>
                </div>
                
                <div className="service-body" style={{ minHeight: "auto", margin: "1.5rem 0", background: "var(--bg-sunken)", padding: "1rem", borderRadius: "var(--radius-md)" }}>
                  ...
                  <div className="row between tiny" style={{ marginTop: "0.75rem" }}>
                    ...
                    <span className="font-bold">{fmtDuration(d.started_at ?? d.created_at, d.finished_at)}</span>
                  </div>
                </div>
                ...
                <AnimatePresence>
                  {expanded.has(d.id) && (
                    ...
                    >
                      {d.build_log ? d.build_log.split("\n").map((line, i) => (
                         <div key={i} className="log-line">
                            <span className="log-msg tiny" style={{ wordBreak: "break-all" }}>{line}</span>
                         </div>
                ...
                <div className="service-footer" style={{ borderTop: "1px solid var(--border-subtle)" }}>
                  <button className="ghost xsmall" onClick={() => setExpanded(prev => { 
                  ...
                  <button className="ghost xsmall" onClick={() => { navigator.clipboard.writeText(d.build_log); toast.success("Logs buffered to clipboard"); }}>
                     <Copy size={12} />
                  </button>
                  {d.status === "failed" && (
                     <button className="ghost xsmall text-danger" style={{ marginLeft: "auto" }}>Retry Build</button>
                  )}
                </div>
              </motion.div>
            ))
