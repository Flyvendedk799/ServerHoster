...
/**
 * Factory for "legacy-database-backed" resource profiles (MySQL, MongoDB).
 *
 ...
 * keeps working on it unchanged, and env() injects DATABASE_URL synchronously
 * from the legacy row.
 */
...
export function buildManagedDbProfile(options: ManagedDbProfileOptions): ResourceProfile {
  const { id, label, engine, drivers, portRange } = options;
    ...
    async plan(ctx: AppContext, serviceId): Promise<ProvisionPlan> {
      ...
      return {
        ...
        actions: [
          {
            id: `create-${id}`,
            label: `Create managed ${label} and inject DATABASE_URL`,
            risk: "safe",
